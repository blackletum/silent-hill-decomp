...
void Dahlia_ControlUpdate(s_SubCharacter* dahlia, GsCOORDINATE2* boneCoords)
{
    ...
    switch (dahliaProps.controlState)
    {
...
#if defined(MAP7_S03)
            WorldGfx_CharaMeshSwap(Chara_EndingDahlia, MESH_SWAP_STATUS(1, 0));
#endif
            Chara_AnimStateReset(dahlia);
            break;
        ...
        case DahliaControl_9:
            ...
            {
                WorldGfx_CharaMeshSwap(Chara_Dahlia, MESH_SWAP_STATUS(1, 0));
            }
            else
            {
#if !defined(MAP6_S01) && !defined(MAP6_S04) && !defined(MAP7_S02) && !defined(MAP7_S03)
#if defined(MAP3_S06)
                WorldGfx_CharaMeshSwap(Chara_Dahlia, MESH_SWAP_STATUS(2, 0));
#else
                WorldGfx_CharaMeshSwap(Chara_Dahlia, MESH_SWAP_STATUS(3, 0));
#endif
#endif
            }
            break;
...
#ifdef MAP7_S03
    WorldGfx_CharaMeshSwap(Chara_EndingDahlia, MESH_SWAP_STATUS(1, 0)); // TODO: MAP7_S03 Dahlia funcs could be EndingDahlia instead, if other Dahlia funcs don't match other overlays.
#else
    WorldGfx_CharaMeshSwap(Chara_Dahlia, MESH_SWAP_STATUS(1, 0));
#endif
